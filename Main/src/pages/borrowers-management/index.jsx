...
const BorrowersManagement = () => {
  ...
  const handleAddBorrower = (borrowerData) => {
    console.log("New borrower data:", borrowerData);
  };

  const handleBlockStatusChange = (borrower) => {
    setBlocklistModal({
      isOpen: true,
      borrowerData: borrower,
      isUnblocking: borrower?.isBlocked,
    });
  };
  ...
  const handleBlocklistConfirm = (data) => {
    console.log(
      ...
      blocklistModal?.borrowerData?.name,
      data?.reason,
    );
    setBlocklistModal({
      isOpen: false,
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 md:mb-8">
        <div>
          ...
          <p className="text-sm md:text-base text-muted-foreground">
            Manage borrower profiles, relationships, and loan portfolios
          </p>
        </div>
        <Button
          variant="default"
