...
const DailyCollectionsWeekly = ({ branchId }) => {
  ...
  const data = (branchData || []).map((item) => ({
    day: item.weekday,
    count: Number(item.active_loans || 0),
    amount: Number(item.total_weekly_amount || 0),
  }));
  ...
  return (
    <div className="mb-6 bg-card border border-border rounded-2xl p-5 md:p-6 shadow-sm">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground mt-0.5">
            Quick view of this week performance
          </p>
        </div>
