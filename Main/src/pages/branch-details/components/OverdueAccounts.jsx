  ...
  return (
    <div className="bg-card rounded-2xl border border-border shadow-sm p-4 md:p-6">
      <div className="flex items-center justify-between mb-4 md:mb-6">
        <h2 className="text-xl md:text-2xl font-semibold text-foreground">Overdue Accounts</h2>
      ...
      <div className="space-y-3 md:space-y-4">
        {accounts?.length > 0 ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
                    <div>
                      ...
                      <p className="text-sm md:text-base font-semibold text-error">
                        Rs {account?.overdueAmount?.toLocaleString("en-IN")}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Days Overdue</p>
                      <p className={`text-sm md:text-base font-semibold ${getDaysOverdueColor(account?.daysOverdue)}`}>
                    ...
                    <div>
                      ...
                      <p className="text-sm md:text-base text-foreground">
                        {new Date(account.lastPaymentDate).toLocaleDateString("en-US", {
                          month: "short",
                          day: "numeric",
                        })}
                      </p>
                    ...
                    <div>
                      ...
                      <p className="text-sm md:text-base text-foreground break-all">{account?.phone}</p>
                    </div>
                  </div>
                </div>
          ...
          <div className="text-center py-8 md:py-12">
            ...
            <p className="text-sm md:text-base text-muted-foreground">No overdue accounts</p>
          </div>
        )}
      </div>
    </div>
